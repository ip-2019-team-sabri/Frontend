...
class integration_project {
    ...
    function maakBezoeker(WP_REST_Request $request) {
        ...
        $userOnUUID = get_users($query); //array of WP_user objects, empty when failed
        $userOnEmail = get_user_by('email', $email); //WP_user object, false when failed

        if (empty($userOnUUID) and $userOnEmail == false) {                    //bezoeker bestaat nog niet
            ...
            if (is_int($user_id)) {
                ...
                add_user_meta($user_id, 'bezoekerUUID', $bezoekerUUID, true);
                add_user_meta($user_id, 'badgeUUID', $badgeUUID, true);
                add_user_meta($user_id, 'adres', $adres, true);
                add_user_meta($user_id, 'btwnummer', $btwnummer, true);
                add_user_meta($user_id, 'bankrekeningnummer', $bankrekeningnummer, true);
                add_user_meta($user_id, 'isAanwezig', $isAanwezig, true);
                add_user_meta($user_id, 'isGeblokkeerd', $isGeblokkeerd, true);
                add_user_meta($user_id, 'isActief', $isActief, true);
                add_user_meta($user_id, 'versienummer', $versienummer, true);
                ...
                return 1; //"Aanmaken bezoeker {$bezoekerUUID} niet gelukt: {$user_id}";
            }
        }
        else {
            if ($userOnEmail != false) $user = $userOnEmail;
            else $user = $userOnUUID[0];

            $user_id = $user->ID;

            $WP_versienummer = (int) get_user_meta($user_id, 'versienummer', true);

            if ($WP_versienummer < $versienummer) { 		//werknemer bestaat maar frontend heeft lager versienummer
                ...
                if (is_int($user_id)) {
                    ...
                    update_user_meta($user_id, 'bezoekerUUID', $bezoekerUUID);
                    update_user_meta($user_id, 'badgeUUID', $badgeUUID);
                    update_user_meta($user_id, 'adres', $adres);
                    update_user_meta($user_id, 'btwnummer', $btwnummer);
                    update_user_meta($user_id, 'bankrekeningnummer', $bankrekeningnummer);
                    update_user_meta($user_id, 'isAanwezig', $isAanwezig);
                    update_user_meta($user_id, 'isGeblokkeerd', $isGeblokkeerd);
                    update_user_meta($user_id, 'isActief', $isActief);
                    update_user_meta($user_id, 'versienummer', $versienummer);
